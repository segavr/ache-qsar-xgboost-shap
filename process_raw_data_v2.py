...
def process_data():
    ...
    # Filter out zero or negative standard_value
    df = df[df['standard_value'] > 0]
    print(f"After dropping NaNs and non-positive values: {len(df)}")
    ...
    # Filter out inf values if any
    df_unique = df_unique[np.isfinite(df_unique['pIC50'])]
